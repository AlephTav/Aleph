...
Генерирует сообщение об ошибке по заданному токену ошибки. Параметры (переменные) шаблона ошибки задаются внутри шаблона в формате PHP функции **sprintf()**. Замена переменных шаблона происходит слева направо.
